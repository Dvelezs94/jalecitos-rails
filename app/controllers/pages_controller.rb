  ...
  def home
    if params[:current]
      ...
      render template: "shared/carousels/add_items_carousel.js.erb"
        ...
        @recent_requests = Request.search("*", where: conditions, order: [{ updated_at: { order: :desc, unmapped_type: :long}}], page: params[:recent_requests], per_page: 5, execute: false)
        @verified_gigs = Gig.search("*", includes: [:gigs_packages, :user], where: conditions("verified"), order: [{ updated_at: { order: :desc, unmapped_type: :long}}], page: params[:verified_gigs], per_page: 5, execute: false)
