class ConversationsController < ApplicationController
  ...
  def set_recipient
    if params[:user_id]
      ...
      if ! @conversation = Conversation.includes(:sender, :recipient, :messages).where("(sender_id = ? OR recipient_id = ?) AND( sender_id = ? OR recipient_id = ?)", current_user.id, current_user.id, @remote_user , @remote_user).first
