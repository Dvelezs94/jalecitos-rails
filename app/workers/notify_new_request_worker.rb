class NotifyNewRequestWorker
  ...
  def perform(request_id)
    ...
    if specif.present? #profession or tags
      # Search from gigs
      @gigs = Gig.search(specif, fields: [:tags, :profession], operator: "or", where: { location: { near: {lat: request.lat, lon: request.lng}, within: "200km" } }, load: false, execute: false, misspellings: {prefix_length: 3})
      # Search from users
      @users = User.search(specif, fields: [:tags], operator: "or", where: { location: { near: {lat: request.lat, lon: request.lng}, within: "200km" } }, load: false, execute: false, misspellings: {prefix_length: 3})
